...
- Shared helpers: src/conformer/, src/qwen3_lm/
...
| qwen3 block (×28) | llm.model.layers.{i} (pre-LN RMSNorm, GQA 16Q/8KV head_dim=128 with per-head Q/K RMSNorm, NeoX RoPE θ=1e6, packed gate_up SwiGLU, intermediate=6144) | [2048, T_total] | `dec.block.{0,14,27}.out` | `transcribe::qwen3_lm::block_prefill()` over BlockView | qwen3_asr/decoder.cpp:185-220 |
...
| Stage | Reference location | Output shape | Gate tensor | ggml / C++ pattern | In-tree analog |
|-------|--------------------|--------------|-------------|--------------------|----------------|
| KV cache layout | self-attn-only, [n_kv_heads · head_dim · n_ctx · n_layer] flat (8 · 128 · ≥1024 · 28) | per-layer K/V slices | n/a | `transcribe::qwen3_lm::kv_init()` + `kv_cache.{n,head}` | qwen3_asr/model.cpp:284-310 (kv_init) |
...
- **No transducer / encoder-decoder cross-attention decoder.** canary's `CanaryDecBlock` (norm1+self / norm2+cross / norm3+ffn) is replaced wholesale by `qwen3_lm::block_prefill` / `block_step` over `qwen3_lm::BlockView`. Decoder hparams come from `stt.canary_qwen.decoder.*` (Qwen3 shape: GQA 16Q/8KV head_dim=128, intermediate=6144, rope_theta=1e6, RMSNorm eps=1e-6, tied embeddings).
...
- **Standard 1D RoPE, NOT MRoPE.** qwen3_asr's RoPE handles MRoPE collapse for text-only; canary_qwen has no MRoPE at all. Pass `rope_mrope_section_*=0` and let `qwen3_lm::block_*` route through the NeoX RoPE path with single-stream positions. (qwen3_lm helper already handles this; the family loader just doesn't read MRoPE KVs.)
