#!/usr/bin/env python3
"""
run_reference_medasr_transformers.py — google/medasr WER baseline.

Mirrors the model card README path (AutoProcessor + AutoModelForCTC +
model.generate + processor.batch_decode(skip_special_tokens=True)) over
a WER manifest. Writes scripts/wer/score.py-compatible JSONL records.

Usage:

    uv run --project scripts/envs/medasr \\
      scripts/wer/run_reference_medasr_transformers.py \\
        --model google/medasr \\
        --revision ae1e4845b4b07479735d93e1e591e566435b7104 \\
        --manifest samples/wer/test-clean.manifest.jsonl \\
        --device mps \\
        --out reports/wer/medasr-REF.test-clean.jsonl
"""
...
def main() -> int:
    ...
    p.add_argument("--batch-size", type=int, default=1,
                   help="Currently a single-utterance loop; reserved for "
                        "future batched inference.")
    ...
    with open(args.out, "w") as fout:
        ...
        for entry in manifest:
            ...
            try:
                ...
                if sr != 16000:
                    raise RuntimeError(
                        f"medasr expects 16kHz; got {sr}Hz"
                    )
                inputs = processor(
                    pcm, sampling_rate=sr, return_tensors="pt", padding=True,
                )
