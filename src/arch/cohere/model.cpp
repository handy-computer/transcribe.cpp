    ...
    const transcribe_run_params * params)
{
    ...
    std::vector<int32_t> tok_buf(n, 0), pos_buf(n, 0);
    std::vector<int64_t> kvidx_buf(n, 0);
    std::vector<int32_t> argmax_buf(n, 0);
    ...
    // Grow the read window (rebuild graph + widen mask) so position `pos` fits.
    ...
    // Prompt feed: prompt_len sequential steps (uniform tokens).
    int pos = 0;
    ...
    if (const char * e = std::getenv("TRANSCRIBE_PERF_DEBUG"); e && *e && *e != '0') {
        std::fprintf(stderr,
            "cohere run_batch: n=%d T_enc_max=%d kv_window=%d (cap %d) prompt=%d\n"
