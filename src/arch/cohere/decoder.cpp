...
ggml_tensor * mha_self_step_batched(
    ...
    ggml_tensor * x, CohereKvCache & kv_cache,
    ...
    int n_heads, int hidden, int il, int max_n_kv, int B)
{
    ...
    // rows at B independent indices kv_idx[b].
    {
        const size_t layer_off_k =
            k_elem * static_cast<size_t>(il) * n_ctx * hidden * B;
        const size_t layer_off_v =
            v_elem * static_cast<size_t>(il) * n_ctx * hidden * B;
        ggml_tensor * k_layer = ggml_view_3d(ctx, kv_cache.self_k,
            hidden, n_ctx, B, k_elem * hidden, k_elem * hidden * n_ctx, layer_off_k);
        ggml_tensor * v_layer = ggml_view_3d(ctx, kv_cache.self_v,
            hidden, n_ctx, B, v_elem * hidden, v_elem * hidden * n_ctx, layer_off_v);
    ...
    ggml_tensor * K = ggml_view_4d(ctx, kv_cache.self_k,
        ...
        k_elem * hidden, k_elem * head_dim, k_elem * hidden * n_ctx, layer_off_k);
    ggml_tensor * V = ggml_view_4d(ctx, kv_cache.self_v,
        head_dim, max_n_kv, n_heads, B,
        v_elem * hidden, v_elem * head_dim, v_elem * hidden * n_ctx, layer_off_v);
...
ggml_tensor * mha_cross_step_batched(
    ggml_context * ctx, ggml_tensor * x, CohereKvCache & kv_cache,
    ...
    int n_heads, int hidden, int il, int T_enc_max, int B)
{
    ...
    const size_t layer_off_k =
        k_elem * static_cast<size_t>(il) * T_enc_max * hidden * B;
    const size_t layer_off_v =
        v_elem * static_cast<size_t>(il) * T_enc_max * hidden * B;
    ggml_tensor * K = ggml_view_4d(ctx, kv_cache.cross_k,
        head_dim, T_enc_max, n_heads, B,
        k_elem * hidden, k_elem * head_dim, k_elem * hidden * T_enc_max, layer_off_k);
    ggml_tensor * V = ggml_view_4d(ctx, kv_cache.cross_v,
        head_dim, T_enc_max, n_heads, B,
        v_elem * hidden, v_elem * head_dim, v_elem * hidden * T_enc_max, layer_off_v);
    ...
    for (int i = 0; i < hp.dec_n_layers; ++i) {
        ...
        }
        {
            ...
            ggml_tensor * co = mha_cross_step_batched(
                ...
                blk.cross_q_w, blk.cross_q_b, blk.cross_out_w, blk.cross_out_b,
