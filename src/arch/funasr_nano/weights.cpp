...
transcribe_status read_funasr_nano_hparams(const gguf_context *  gguf,
                                           FunAsrNanoHParams &   hp)
{
    if (gguf == nullptr) {
    ...
    // Encoder.
    if (auto st = read_required_u32_kv(gguf, "stt.funasr_nano.encoder.n_blocks",   kFamilyTag, hp.enc_n_blocks);    st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv(gguf, "stt.funasr_nano.encoder.tp_blocks",  kFamilyTag, hp.enc_tp_blocks);   st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv(gguf, "stt.funasr_nano.encoder.d_model",    kFamilyTag, hp.enc_d_model);     st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv(gguf, "stt.funasr_nano.encoder.d_input",    kFamilyTag, hp.enc_d_input);     st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv(gguf, "stt.funasr_nano.encoder.n_heads",    kFamilyTag, hp.enc_n_heads);     st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv(gguf, "stt.funasr_nano.encoder.d_ff",       kFamilyTag, hp.enc_d_ff);        st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv(gguf, "stt.funasr_nano.encoder.kernel_size",kFamilyTag, hp.enc_kernel);      st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv(gguf, "stt.funasr_nano.encoder.sanm_shift", kFamilyTag, hp.enc_sanm_shift);  st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_string_kv(gguf, "stt.funasr_nano.encoder.attention_type", kFamilyTag, hp.enc_attn_type); st != TRANSCRIBE_OK) return st;
    if (auto st = read_optional_bool_kv(gguf, "stt.funasr_nano.encoder.normalize_before", kFamilyTag, true, hp.enc_normalize_before); st != TRANSCRIBE_OK) return st;

    // Adaptor.
    if (auto st = read_required_u32_kv(gguf, "stt.funasr_nano.adaptor.n_blocks",      kFamilyTag, hp.adaptor_n_blocks);      st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv(gguf, "stt.funasr_nano.adaptor.encoder_dim",   kFamilyTag, hp.adaptor_encoder_dim);   st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv(gguf, "stt.funasr_nano.adaptor.llm_dim",       kFamilyTag, hp.adaptor_llm_dim);       st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv(gguf, "stt.funasr_nano.adaptor.pre_ffn_dim",   kFamilyTag, hp.adaptor_pre_ffn_dim);   st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv(gguf, "stt.funasr_nano.adaptor.block_ffn_dim", kFamilyTag, hp.adaptor_block_ffn_dim); st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv(gguf, "stt.funasr_nano.adaptor.n_heads",       kFamilyTag, hp.adaptor_n_heads);       st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv(gguf, "stt.funasr_nano.adaptor.d_head",        kFamilyTag, hp.adaptor_d_head);        st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_f32_kv(gguf, "stt.funasr_nano.adaptor.layer_norm_eps", kFamilyTag, hp.adaptor_layer_norm_eps); st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_string_kv(gguf, "stt.funasr_nano.adaptor.activation", kFamilyTag, hp.adaptor_activation); st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv(gguf, "stt.funasr_nano.adaptor.downsample_rate", kFamilyTag, hp.adaptor_downsample_rate); st != TRANSCRIBE_OK) return st;
    if (auto st = read_optional_bool_kv(gguf, "stt.funasr_nano.adaptor.use_low_frame_rate", kFamilyTag, true, hp.adaptor_use_low_frame_rate); st != TRANSCRIBE_OK) return st;

    // Decoder (Qwen3-0.6B).
    if (auto st = read_required_u32_kv(gguf, "stt.funasr_nano.decoder.n_layers",        kFamilyTag, hp.dec_n_layers);     st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv(gguf, "stt.funasr_nano.decoder.hidden_size",     kFamilyTag, hp.dec_hidden);       st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv(gguf, "stt.funasr_nano.decoder.intermediate_size",kFamilyTag, hp.dec_intermediate); st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv(gguf, "stt.funasr_nano.decoder.n_heads",         kFamilyTag, hp.dec_n_heads);      st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv(gguf, "stt.funasr_nano.decoder.n_kv_heads",      kFamilyTag, hp.dec_n_kv_heads);   st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv(gguf, "stt.funasr_nano.decoder.head_dim",        kFamilyTag, hp.dec_head_dim);     st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv(gguf, "stt.funasr_nano.decoder.vocab_size",      kFamilyTag, hp.dec_vocab_size);   st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv(gguf, "stt.funasr_nano.decoder.max_position_embeddings", kFamilyTag, hp.dec_max_position_embeddings); st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_f32_kv(gguf, "stt.funasr_nano.decoder.rms_norm_eps",    kFamilyTag, hp.dec_rms_norm_eps); st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_f32_kv(gguf, "stt.funasr_nano.decoder.rope_theta",      kFamilyTag, hp.dec_rope_theta);   st != TRANSCRIBE_OK) return st;
    if (auto st = read_optional_bool_kv(gguf, "stt.funasr_nano.decoder.tie_word_embeddings", kFamilyTag, true, hp.dec_tie_word_embeddings); st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_string_kv(gguf, "stt.funasr_nano.decoder.activation",   kFamilyTag, hp.dec_activation);   st != TRANSCRIBE_OK) return st;
    ...
    if (auto st = read_optional_bool_kv(gguf,   "stt.frontend.apply_cmvn",kFamilyTag, false, hp.fe_apply_cmvn); st != TRANSCRIBE_OK) return st;
                                            ...
                                            FunAsrNanoWeights &        weights)
{
    ...
    for (int i = 0; i < hp.enc_n_blocks - 1; ++i) {
        ...
        if (auto st = load_enc_block(ctx_meta, prefix, d_model, d_model,
                                     d_ff, kernel, weights.encoders[i]);
            st != TRANSCRIBE_OK) return st;
    }
    ...
    for (int i = 0; i < hp.enc_tp_blocks; ++i) {
        ...
        if (auto st = load_enc_block(ctx_meta, prefix, d_model, d_model,
                                     d_ff, kernel, weights.tp_encoders[i]);
            st != TRANSCRIBE_OK) return st;
    }
    ...
    for (int i = 0; i < hp.adaptor_n_blocks; ++i) {
        ...
        if (auto st = load_adaptor_block(ctx_meta, prefix, llm_dim,
                                         block_ffn_dim,
                                         weights.adaptor.blocks[i]);
            st != TRANSCRIBE_OK) return st;
    }
