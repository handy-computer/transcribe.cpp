...
ggml_tensor * causal_conv(ggml_context * ctx, ggml_tensor * kernel, ggml_tensor * x,
...
ggml_tensor * mha(ggml_context * ctx, ggml_tensor * x,
    ...
    Q = ggml_rope_ext(ctx, Q, positions, nullptr, head_dim, GGML_ROPE_TYPE_NEOX,
                      max_pos, rope_theta, 1.0f, 0.0f, 1.0f, 32.0f, 1.0f);
    K = ggml_rope_ext(ctx, K, positions, nullptr, head_dim, GGML_ROPE_TYPE_NEOX,
                      max_pos, rope_theta, 1.0f, 0.0f, 1.0f, 32.0f, 1.0f);
...
ggml_tensor * mha_cached(ggml_context * ctx, ggml_cgraph * gf, ggml_tensor * x,
    ...
    const size_t write_off =
        (static_cast<size_t>(layer) * n_ctx + static_cast<size_t>(write_slot)) * kv_dim;
    ...
    const size_t read_off =
        (static_cast<size_t>(layer) * n_ctx + static_cast<size_t>(read_start)) * kv_dim;
    ggml_tensor * K_att = ggml_view_3d(ctx, kv.self_k, head_dim, read_len, n_heads,
                                       k_elem * kv_dim, k_elem * head_dim, k_elem * read_off);
    ggml_tensor * V_att = ggml_view_3d(ctx, kv.self_v, head_dim, read_len, n_heads,
                                       v_elem * kv_dim, v_elem * head_dim, v_elem * read_off);
...
EncoderBuild build_encoder_graph(ggml_context * ctx,
    ...
    for (int i = 0; i < n_blocks; ++i) {
        const EncBlock & b = w.enc_blocks[i];
        {
            ...
            y = mha(ctx, y, b, eb.positions_in, eb.mask_in,
                    n_heads, head_dim, d_model, hp.enc_rope_theta, hp.enc_max_pos, use_flash);
            x = ggml_add(ctx, x, y);
        }
        {
    ...
    if (eb.dumps.embedder_out) ggml_build_forward_expand(eb.graph, eb.dumps.embedder_out);
    if (eb.dumps.enc_out)      ggml_build_forward_expand(eb.graph, eb.dumps.enc_out);
    for (ggml_tensor * t : eb.dumps.block_outs) ggml_build_forward_expand(eb.graph, t);
...
EncoderBuildBatched build_encoder_graph_batched(ggml_context * ctx,
    ...
    for (int i = 0; i < n_blocks; ++i) {
        ...
        {
            ggml_tensor * y = rms_norm(ctx, x, b.norm_attn_w, hp.enc_rms_norm_eps);
            y = mha_batched(ctx, y, b, eb.positions_in, eb.mask_in,
                            n_heads, head_dim, d_model, hp.enc_rope_theta, hp.enc_max_pos,
                            use_flash);
            x = ggml_add(ctx, x, y);
        }
        {
    ...
    if (T_enc != n_audio * hp.proj_downsample) {
        xc = ggml_view_3d(ctx, xc, d_model,
                          static_cast<int64_t>(n_audio) * hp.proj_downsample, B,
                          xc->nb[1], xc->nb[2], 0);
        xc = ggml_cont(ctx, xc);
    }
...
EmbedderChunkBuild build_embedder_chunk_graph(ggml_context * ctx,
    ...
    named(eb.mel_in, "enc.mel.chunk.in");  ggml_set_input(eb.mel_in);
    eb.cache1_in = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n_mels, 2);   // conv0 left_pad
    named(eb.cache1_in, "enc.conv0.cache"); ggml_set_input(eb.cache1_in);
    eb.cache2_in = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, d_model, 1);  // conv1 left_pad
    named(eb.cache2_in, "enc.conv1.cache"); ggml_set_input(eb.cache2_in);
    ...
    ggml_tensor * c0_last = ggml_view_2d(ctx, conv0_out, 1, d_model,
                                         conv0_out->nb[1],
                                         static_cast<size_t>(n_new_mel - 1) * conv0_out->nb[0]); // [1, d_model]
    eb.cache2_out = ggml_cont(ctx, ggml_transpose(ctx, c0_last));                    // [d_model, 1]
    named(eb.cache2_out, "enc.conv1.cache.next"); ggml_set_output(eb.cache2_out);
    ...
    x = conv_1d_f32(ctx, w.enc_stem.conv1_w, conv0_full, /*stride=*/2, /*padding=*/0, /*dilation=*/1); // [M/2, d_model]
    x = add_conv1d_bias(ctx, x, w.enc_stem.conv1_b);
    x = ggml_gelu_erf(ctx, x);
    eb.out = ggml_cont(ctx, ggml_transpose(ctx, x));                                 // [d_model, M_emb]
    named(eb.out, "enc.embedder.chunk.out"); ggml_set_output(eb.out);
...
EncoderChunkBuild build_encoder_chunk_graph(ggml_context *                  ctx,
    ...
    cb.n_new = n_new; cb.read_len = read_len; cb.n_audio_new = n_audio;
    ...
    for (int i = 0; i < n_blocks; ++i) {
        ...
        {
            ggml_tensor * y = rms_norm(ctx, x, b.norm_attn_w, hp.enc_rms_norm_eps);
            y = mha_cached(ctx, gf, y, b, cb.positions_in, cb.mask_in, enc_kv, i,
                           n_new, write_slot, read_start, read_len, n_heads, head_dim,
                           hp.enc_rope_theta, hp.enc_max_pos, use_flash);
            x = ggml_add(ctx, x, y);
        }
        {
            ggml_tensor * y = rms_norm(ctx, x, b.norm_ffn_w, hp.enc_rms_norm_eps);
