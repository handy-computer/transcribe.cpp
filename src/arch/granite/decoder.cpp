                                 ...
                                 bool                            slice_last)
{
    ...
    pb.audio_feats_in = ggml_new_tensor_2d(ctx, GGML_TYPE_F32,
                                           hidden, n_audio_tokens);
    ...
    if (suffix_len > 0) {
        x_suffix = ggml_view_2d(
            ...
            emb_elem * hidden,
            emb_elem * hidden *
                static_cast<size_t>(prefix_len + n_audio_tokens));
    ...
    ggml_tensor * head_w = (weights.dec_final.output_w != nullptr)
                            ? weights.dec_final.output_w
                            : weights.dec_embed.token_w;
