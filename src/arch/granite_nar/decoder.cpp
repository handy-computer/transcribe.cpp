                         ...
                         ggml_tensor *              positions)
{
    ...
    if (n_groups == 1) {
        ...
        ggml_tensor * K_tpl = ggml_new_tensor_4d(ctx, K_att->type,
                                                 head_dim, T_total, n_groups, n_kv_heads);
        ggml_tensor * V_tpl = ggml_new_tensor_4d(ctx, V_att->type,
                                                 head_dim, T_total, n_groups, n_kv_heads);
    ...
    o = ggml_reshape_2d(ctx, o,
                        static_cast<int64_t>(n_heads) * head_dim, T_total);
                                 ...
                                 int                       n_text)
{
    ...
    ggml_tensor * text_emb = ggml_get_rows(
        ctx, weights.dec_embed.token_w, fb.text_ids_in);
    ...
    if (fb.dumps.flat_embeds)  ggml_build_forward_expand(gf, fb.dumps.flat_embeds);
                              ...
                              std::vector<int32_t> &     out_ids)
{
    ...
    for (int i = 0; i < n_text; ++i) {
        ...
        if (best == eos_id) { prev = best; continue; }
