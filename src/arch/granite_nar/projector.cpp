                                     ...
                                     int                       T_enc)
{
    ...
    for (int j = 0; j < enc_layers; ++j) {
        ggml_tensor * slc = ggml_view_2d(
            ctx, pb.enc_in,
            enc_hidden, T_enc,
            pb.enc_in->nb[1],
            cat_in_bytes * j);
        slc = ggml_cont(ctx, slc);
        slc = layer_norm(ctx, slc,
                         weights.proj_top.layer_norms_w[j],
                         weights.proj_top.layer_norms_b[j],
                         ln_eps);
        if (normed == nullptr) normed = slc;
        else normed = ggml_concat(ctx, normed, slc, /*dim=*/0);
    }
    ...
    if (pb.t_enc_pad > 0) {
        pb.enc_pad = ggml_new_tensor_2d(
            ctx, GGML_TYPE_F32, prj_hidden, pb.t_enc_pad);
    ...
    ggml_tensor * wpos = ggml_cast(ctx, weights.proj_top.window_positions,
                                   GGML_TYPE_F32);
    ...
    ggml_tensor * mp_view = ggml_reshape_4d(ctx, windowed,
                                            prj_hidden, downsample, n_query, pb.nblocks);
    ...
    for (int i = 0; i < hp.prj_n_layers; ++i) {
        ...
        ggml_tensor * attn   = cross_attn(ctx, q_proj, k_proj, v_proj,
                                          n_heads, head_dim);
    ...
    ggml_tensor * tokens = ggml_reshape_2d(ctx, onorm,
                                           prj_hidden,
                                           static_cast<int64_t>(n_query) * pb.nblocks);
    ...
    if (pb.dumps.qformer_out) ggml_build_forward_expand(pb.graph, pb.dumps.qformer_out);
