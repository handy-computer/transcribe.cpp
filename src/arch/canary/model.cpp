...
transcribe_status load(Loader & loader, const transcribe_model_load_params * params, transcribe_model ** out_model) {
    ...
    if (canary_supports_long_form(*m)) {
...
transcribe_status run(transcribe_session *          session,
    ...
    for (const CanaryChunkSpan & span : chunks) {
        ...
        // Drop per-window ggml scratch before the next 30-40 s encode.
        // The scheduler allocator only grows; keeping it across windows
        // would pin peak memory for the whole clip (see #150).
        cc->release_scratch();
