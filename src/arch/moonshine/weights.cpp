                                         ...
                                         MoonshineHParams &   hp)
{
    ...
    // Encoder.
    if (auto st = read_required_u32_kv(gguf, "stt.moonshine.encoder.n_layers",   kFamilyTag, hp.enc_n_layers);   st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv(gguf, "stt.moonshine.encoder.d_model",    kFamilyTag, hp.enc_d_model);    st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv(gguf, "stt.moonshine.encoder.n_heads",    kFamilyTag, hp.enc_n_heads);    st != TRANSCRIBE_OK) return st;
    if (auto st = read_optional_u32_kv(gguf, "stt.moonshine.encoder.n_kv_heads", kFamilyTag, hp.enc_n_heads, hp.enc_n_kv_heads); st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv(gguf, "stt.moonshine.encoder.ffn_dim",    kFamilyTag, hp.enc_ffn_dim);    st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_string_kv(gguf, "stt.moonshine.encoder.activation", kFamilyTag, hp.enc_activation); st != TRANSCRIBE_OK) return st;

    // Decoder.
    if (auto st = read_required_u32_kv(gguf, "stt.moonshine.decoder.n_layers",                kFamilyTag, hp.dec_n_layers);                st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv(gguf, "stt.moonshine.decoder.d_model",                 kFamilyTag, hp.dec_d_model);                 st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv(gguf, "stt.moonshine.decoder.n_heads",                 kFamilyTag, hp.dec_n_heads);                 st != TRANSCRIBE_OK) return st;
    if (auto st = read_optional_u32_kv(gguf, "stt.moonshine.decoder.n_kv_heads",              kFamilyTag, hp.dec_n_heads, hp.dec_n_kv_heads); st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv(gguf, "stt.moonshine.decoder.ffn_dim",                 kFamilyTag, hp.dec_ffn_dim);                 st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_string_kv(gguf, "stt.moonshine.decoder.activation",           kFamilyTag, hp.dec_activation);              st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv(gguf, "stt.moonshine.decoder.vocab_size",              kFamilyTag, hp.dec_vocab_size);              st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv(gguf, "stt.moonshine.decoder.max_position_embeddings", kFamilyTag, hp.dec_max_position_embeddings); st != TRANSCRIBE_OK) return st;
    if (auto st = read_optional_bool_kv(gguf, "stt.moonshine.decoder.tie_word_embeddings",    kFamilyTag, true, hp.dec_tie_word_embeddings); st != TRANSCRIBE_OK) return st;

    // Special tokens.
    if (auto st = read_required_u32_kv(gguf, "stt.moonshine.bos_token_id",           kFamilyTag, hp.bos_token_id);           st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv(gguf, "stt.moonshine.eos_token_id",           kFamilyTag, hp.eos_token_id);           st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv(gguf, "stt.moonshine.pad_token_id",           kFamilyTag, hp.pad_token_id);           st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv(gguf, "stt.moonshine.decoder_start_token_id", kFamilyTag, hp.decoder_start_token_id); st != TRANSCRIBE_OK) return st;

    // Attention / RoPE.
    if (auto st = read_optional_f32_kv (gguf, "stt.moonshine.partial_rotary_factor",       kFamilyTag, 0.9f, hp.partial_rotary_factor); st != TRANSCRIBE_OK) return st;
    if (auto st = read_optional_f32_kv (gguf, "stt.moonshine.rope_theta",                  kFamilyTag, 10000.0f, hp.rope_theta);        st != TRANSCRIBE_OK) return st;
    if (auto st = read_optional_bool_kv(gguf, "stt.moonshine.attention_bias",              kFamilyTag, false, hp.attention_bias);       st != TRANSCRIBE_OK) return st;
    if (auto st = read_optional_u32_kv (gguf, "stt.moonshine.pad_head_dim_to_multiple_of", kFamilyTag, 0, hp.pad_head_dim_multiple);    st != TRANSCRIBE_OK) return st;

    // Conv stem.
    if (auto st = read_required_u32_array_kv(gguf, "stt.moonshine.conv_stem.channels",     kFamilyTag, hp.conv_channels);     st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_array_kv(gguf, "stt.moonshine.conv_stem.kernel_sizes", kFamilyTag, hp.conv_kernel_sizes); st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_array_kv(gguf, "stt.moonshine.conv_stem.strides",      kFamilyTag, hp.conv_strides);      st != TRANSCRIBE_OK) return st;
    if (auto st = read_optional_u32_kv (gguf, "stt.moonshine.conv_stem.groupnorm_num_groups", kFamilyTag, 1, hp.conv_groupnorm_num_groups); st != TRANSCRIBE_OK) return st;
    if (auto st = read_optional_f32_kv (gguf, "stt.moonshine.conv_stem.groupnorm_eps",        kFamilyTag, 1e-5f, hp.conv_groupnorm_eps);    st != TRANSCRIBE_OK) return st;
