                                             ...
                                             std::vector<int32_t> & out)
{
    ...
    if (elem_type == GGUF_TYPE_UINT32) {
        ...
        for (size_t i = 0; i < n; ++i) out[i] = static_cast<int32_t>(src[i]);
    } else {
                                                    ...
                                                    MoonshineStreamingHParams &        hp)
{
    ...
    // Encoder.
    if (auto st = read_required_u32_kv (gguf, "stt.moonshine_streaming.encoder.n_layers",   kFamilyTag, hp.enc_n_layers);   st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv (gguf, "stt.moonshine_streaming.encoder.d_model",    kFamilyTag, hp.enc_d_model);    st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv (gguf, "stt.moonshine_streaming.encoder.n_heads",    kFamilyTag, hp.enc_n_heads);    st != TRANSCRIBE_OK) return st;
    if (auto st = read_optional_u32_kv (gguf, "stt.moonshine_streaming.encoder.n_kv_heads", kFamilyTag, hp.enc_n_heads, hp.enc_n_kv_heads); st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv (gguf, "stt.moonshine_streaming.encoder.head_dim",   kFamilyTag, hp.enc_head_dim);   st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv (gguf, "stt.moonshine_streaming.encoder.ffn_dim",    kFamilyTag, hp.enc_ffn_dim);    st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_string_kv(gguf, "stt.moonshine_streaming.encoder.activation", kFamilyTag, hp.enc_activation); st != TRANSCRIBE_OK) return st;
    {
        float fr = 0.0f;
        if (auto st = transcribe::read_required_f32_kv(gguf, "stt.moonshine_streaming.encoder.frame_ms", kFamilyTag, fr); st != TRANSCRIBE_OK) return st;
        hp.enc_frame_ms = fr;
    }
    if (auto st = read_required_u32_kv (gguf, "stt.moonshine_streaming.encoder.frame_len",   kFamilyTag, hp.enc_frame_len); st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_array_kv(gguf, "stt.moonshine_streaming.encoder.sliding_windows", kFamilyTag, hp.enc_sliding_windows); st != TRANSCRIBE_OK) return st;

    // Decoder.
    if (auto st = read_required_u32_kv (gguf, "stt.moonshine_streaming.decoder.n_layers",                kFamilyTag, hp.dec_n_layers);                st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv (gguf, "stt.moonshine_streaming.decoder.d_model",                 kFamilyTag, hp.dec_d_model);                 st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv (gguf, "stt.moonshine_streaming.decoder.n_heads",                 kFamilyTag, hp.dec_n_heads);                 st != TRANSCRIBE_OK) return st;
    if (auto st = read_optional_u32_kv (gguf, "stt.moonshine_streaming.decoder.n_kv_heads",              kFamilyTag, hp.dec_n_heads, hp.dec_n_kv_heads); st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv (gguf, "stt.moonshine_streaming.decoder.head_dim",                kFamilyTag, hp.dec_head_dim);                st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv (gguf, "stt.moonshine_streaming.decoder.ffn_dim",                 kFamilyTag, hp.dec_ffn_dim);                 st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_string_kv(gguf, "stt.moonshine_streaming.decoder.activation",            kFamilyTag, hp.dec_activation);              st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv (gguf, "stt.moonshine_streaming.decoder.vocab_size",              kFamilyTag, hp.dec_vocab_size);              st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv (gguf, "stt.moonshine_streaming.decoder.max_position_embeddings", kFamilyTag, hp.dec_max_position_embeddings); st != TRANSCRIBE_OK) return st;
    if (auto st = read_optional_bool_kv(gguf, "stt.moonshine_streaming.decoder.tie_word_embeddings",     kFamilyTag, false, hp.dec_tie_word_embeddings); st != TRANSCRIBE_OK) return st;

    // Special tokens.
    if (auto st = read_required_u32_kv(gguf, "stt.moonshine_streaming.bos_token_id",           kFamilyTag, hp.bos_token_id);           st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv(gguf, "stt.moonshine_streaming.eos_token_id",           kFamilyTag, hp.eos_token_id);           st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv(gguf, "stt.moonshine_streaming.pad_token_id",           kFamilyTag, hp.pad_token_id);           st != TRANSCRIBE_OK) return st;
    if (auto st = read_required_u32_kv(gguf, "stt.moonshine_streaming.decoder_start_token_id", kFamilyTag, hp.decoder_start_token_id); st != TRANSCRIBE_OK) return st;

    // Attention / RoPE.
    if (auto st = read_optional_f32_kv (gguf, "stt.moonshine_streaming.partial_rotary_factor",       kFamilyTag, 0.8f, hp.partial_rotary_factor); st != TRANSCRIBE_OK) return st;
    if (auto st = read_optional_f32_kv (gguf, "stt.moonshine_streaming.rope_theta",                  kFamilyTag, 10000.0f, hp.rope_theta);        st != TRANSCRIBE_OK) return st;
    if (auto st = read_optional_bool_kv(gguf, "stt.moonshine_streaming.attention_bias",              kFamilyTag, false, hp.attention_bias);       st != TRANSCRIBE_OK) return st;
    if (auto st = read_optional_u32_kv (gguf, "stt.moonshine_streaming.pad_head_dim_to_multiple_of", kFamilyTag, 0, hp.pad_head_dim_multiple);    st != TRANSCRIBE_OK) return st;

    // CMVN epsilon (informational; the value is fixed in modeling code at 1e-6).
    if (auto st = read_optional_f32_kv (gguf, "stt.moonshine_streaming.cmvn_eps", kFamilyTag, 1e-6f, hp.cmvn_eps); st != TRANSCRIBE_OK) return st;

    // Adapter.
    if (auto st = read_optional_u32_kv (gguf, "stt.moonshine_streaming.encoder_hidden_size", kFamilyTag, hp.enc_d_model, hp.encoder_hidden_size); st != TRANSCRIBE_OK) return st;
    if (auto st = read_optional_bool_kv(gguf, "stt.moonshine_streaming.adapter_has_proj",    kFamilyTag, false, hp.adapter_has_proj);             st != TRANSCRIBE_OK) return st;
    ...
    if (auto st = read_optional_bool_kv(gguf, "stt.capability.streaming",   kFamilyTag, true,  hp.cap_streaming);   st != TRANSCRIBE_OK) return st;
    ...
    {
        std::fprintf(stderr, "moonshine_streaming: decoder hparams must be positive\n");
        return TRANSCRIBE_ERR_GGUF;
    }
    if (hp.dec_d_model != hp.dec_n_heads * hp.dec_head_dim) {
        std::fprintf(stderr,
                     "moonshine_streaming: decoder d_model (%d) != n_heads (%d) * head_dim (%d)\n",
    ...
    if (hp.attention_bias) {
        std::fprintf(stderr,
                     "moonshine_streaming: attention_bias=true is not supported\n");
        return TRANSCRIBE_ERR_GGUF;
    }
    if (hp.partial_rotary_factor <= 0.0f || hp.partial_rotary_factor > 1.0f) {
        std::fprintf(stderr,
                     "moonshine_streaming: invalid partial_rotary_factor=%g (expected (0, 1])\n",
                     hp.partial_rotary_factor);
        return TRANSCRIBE_ERR_GGUF;
    }
    if (hp.fe_type != "raw") {
        std::fprintf(stderr,
                     "moonshine_streaming: unsupported frontend type \"%s\" (only \"raw\")\n",
                     hp.fe_type.c_str());
        return TRANSCRIBE_ERR_GGUF;
    }
    if (hp.fe_sample_rate != 16000) {
        std::fprintf(stderr,
                     "moonshine_streaming: unsupported sample_rate=%d (only 16000 Hz)\n",
                     hp.fe_sample_rate);
        return TRANSCRIBE_ERR_GGUF;
    }
    if (hp.enc_frame_len <= 0) {
        std::fprintf(stderr,
                     "moonshine_streaming: invalid encoder frame_len=%d\n",
                     hp.enc_frame_len);
        return TRANSCRIBE_ERR_GGUF;
    }
    if (hp.enc_sliding_windows.size() !=
        static_cast<size_t>(hp.enc_n_layers) * 2)
    {
        std::fprintf(stderr,
    ...
    {
        std::fprintf(stderr, "moonshine_streaming: special token IDs must be set\n");
        return TRANSCRIBE_ERR_GGUF;
    }
