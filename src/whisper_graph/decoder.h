...
// Optional cross-attention capture, for families that align words against
// supervised cross-attention (crisperwhisper's Viterbi timer). Null / empty
// means no capture and leaves the graph byte-identical, which is what the
// whisper family passes.
//
// Listing a layer forces that layer's cross-attention onto the manual
// mul_mat + soft_max path, because ggml_flash_attn_ext fuses the softmax and
// never materializes the probabilities. Only the listed layers pay that cost.
using AlignHeads = std::vector<std::pair<int, int>>;  // (decoder layer, head)
...
struct DecoderBuild {
    ...
    // Head-averaged cross-attention for the LAST query row, [T_enc_pad].
    // Non-null only when align_heads was supplied.
    ggml_tensor * cross_align = nullptr;
...
struct StepBuild {
    ...
    ggml_tensor * cross_align   = nullptr;  // f32 [T_enc_pad], null unless captured
    int           max_n_kv      = 0;
    ggml_cgraph * graph         = nullptr;
};
