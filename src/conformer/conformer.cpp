                          ...
                          const BlockParams & params)
{
    ...
    if (policy.direct_pw) {
        ...
        // dispatches MMQ).
        {
            // The weight ships conv-shaped [1, d_model, 2*d_model] and
            // is reshaped to its byte-identical 2-D matmul view. The
            // CPU repack load transform replaces it with a native 2-D
            // tensor (interleaved layout, must NOT be viewed through
            // reshape — views drop the repack tensor traits); consume
            // that directly.
            ggml_tensor * pw1 = ggml_n_dims(b.conv_pw1_w) == 2
                ? b.conv_pw1_w
                : ggml_reshape_2d(ctx, b.conv_pw1_w,
                                  d_model, 2 * d_model);
    ...
    if (policy.direct_pw) {
        ...
        // and the 2-D repack replacement rationale.
        ggml_tensor * pw2 = ggml_n_dims(b.conv_pw2_w) == 2
            ? b.conv_pw2_w
            : ggml_reshape_2d(ctx, b.conv_pw2_w,
                              d_model, d_model);
                                    ...
                                    const BlockObserver * obs)
{
    ...
    // yet at this point).
    {
        ...
        if (streaming && !kv_mode) {
            ...
            // x_norm. With streaming_q_slice (default) the pre-concat
            // x_norm doubles as the query-only input — attention output
            // is computed for just the T_q_new new frames while K/V
            // span the full virtual window. Without it, rel_pos_mhsa
            // runs the historical full T_virtual x T_virtual attention
            // and the output is sliced below.
            x_q_new = params.streaming_q_slice ? x_norm : nullptr;
