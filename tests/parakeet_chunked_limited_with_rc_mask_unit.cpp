...
std::vector<bool> reference_mask(int T, int L, int C, int R, int P) {
    ...
    for (int q = 0; q < T; ++q) {
        ...
        for (int k = 0; k < T; ++k) {
            ...
            mask[static_cast<size_t>(q) * T + k] =
                (k >= ws && k <= we) && !q_padded && !k_padded;
        }
    }
    return mask;
}
    ...
    for (size_t i = 0; i < got.size(); ++i) {
        ...
        if (!well_formed) {
            ...
            if (mismatches <= 4) {
                ...
                std::fprintf(stderr,
                             "  [%s] mask[%d, %d] = %f (expected 0.0 or -INF)\n",
                             name, q, k, cell);
            }
            continue;
        }
        if (allowed_ref != allowed_got) {
            ...
            if (mismatches <= 4) {
                ...
                const int k = static_cast<int>(i % T);
                std::fprintf(stderr,
                             "  [%s] mask[%d, %d] = %s but ref says %s\n",
                             name, q, k,
                             allowed_got ? "0.0 (allowed)" : "-INF (masked)",
                             allowed_ref ? "allowed" : "masked");
            }
        }
    }
    if (mismatches == 0) {
        ...
        std::fprintf(stderr,
                     "  [%s] FAIL T=%d L=%d C=%d R=%d — %d mismatching cells\n",
