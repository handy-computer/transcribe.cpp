// cohere_e2e_smoke.cpp - real-model gated end-to-end test for the Cohere ASR
...
int main() {
    ...
    std::fprintf(stderr, "cohere_e2e_smoke: model=%s\n", model_path.c_str());
    std::fprintf(stderr, "cohere_e2e_smoke: audio=%s\n", wav_path.c_str());
    ...
    std::fprintf(stderr, "cohere_e2e_smoke: text=\"%s\"\n", actual.c_str());
    ...
    if (g_failures > 0) {
        std::fprintf(stderr, "cohere_e2e_smoke: %d failures\n", g_failures);
        return EXIT_FAILURE;
    }
    std::fprintf(stdout, "cohere_e2e_smoke: ok\n");
    return EXIT_SUCCESS;
}
