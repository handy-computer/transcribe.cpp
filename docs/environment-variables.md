...
| `TRANSCRIBE_NO_GPU_DENYLIST` | Disable the AUTO GPU denylist: `TRANSCRIBE_BACKEND_AUTO` considers every GPU again, including integrated GPUs known to be slower than the CPU for ASR (pre-Xe Intel, GCN 1-3 AMD APUs, 1-2 CU AMD display adapters; see `transcribe_backend_request` in `include/transcribe.h`). Devices are identified by PCI vendor:device id via the Vulkan backend (`patches/ggml/0002-vulkan-expose-device-hw-info.patch`); the id sets live in `src/transcribe-gpu-denylist-data.h`, generated by `scripts/gen_gpu_denylist.py`. Explicit backend/device selection never consults the denylist, so this is only needed to change AUTO's answer. |
| `TRANSCRIBE_TEST_GPU_HW_IDS=<vendor>:<device>[:<cores>]` | Test hook: every GPU device reports the given hardware identity (hex PCI ids, decimal shader-core count) and counts as integrated for the denylist, so the AUTO skip → CPU path can be exercised on any machine with a GPU (e.g. `8086:5917` impersonates a UHD 620). Malformed values are ignored with a warning. Used by `backend_gpu_denylist_gate_unit`. |
