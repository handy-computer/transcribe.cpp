...
| nemotron-3.5-asr-streaming-0.6b | Transcribe (offline / cache-aware att_context_size=[56,13], 1.12s chunk) | explicit en | `build/bin/transcribe-cli -m models/nemotron-3.5-asr-streaming-0.6b/nemotron-3.5-asr-streaming-0.6b-F32.gguf --language en samples/jfk.wav` | English transcript with PnC | TODO (Target: MUST PASS) |
| nemotron-3.5-asr-streaming-0.6b | Transcribe (offline) — non-English explicit language | explicit es (or de) | `build/bin/transcribe-cli -m <gguf> --language es samples/<es-clip>.wav` | Spanish transcript with PnC matching reference | TODO (Target: MUST PASS — pending user sign-off; multilingual is the headline capability of this checkpoint) |
| nemotron-3.5-asr-streaming-0.6b | Auto language detection (`target_lang=auto`) | auto | `build/bin/transcribe-cli -m <gguf> --language auto samples/jfk.wav` | English transcript; runtime exposes either a `<lang-XX>` tag or a structured detected-language field per CLI design | TODO (Target: MUST PASS — pending user sign-off; advertised on card and the only no-hint mode) |
| nemotron-3.5-asr-streaming-0.6b | Punctuation/casing | output | same as the explicit-en row | output contains capital letters and `,.?!` | TODO (Target: MUST PASS — pending user sign-off; native PnC is advertised) |
| nemotron-3.5-asr-streaming-0.6b | Streaming (cache reuse across chunks) | streaming | `build/bin/transcribe-cli -m <gguf> --language en --backend cpu --threads 1 --stream-chunk-ms 1120 --stream-att-right 13 samples/jfk.wav` | byte-equal transcript vs one-shot at the default `att_context_size=[56,13]` (1.12s chunk) | TODO (Target: MUST PASS — forced by intake `capabilities.streaming: true`) |
| nemotron-3.5-asr-streaming-0.6b | Other latency settings ([56,0]/[56,1]/[56,3]/[56,6]/[56,13]) | runtime-selectable att_context_size | `… --stream-chunk-ms <ms> --stream-att-right {0,1,3,6,13} …` (R selects the right-context from the training menu; 5 settings on this checkpoint vs 4 on the predecessor) | all five R settings stream a valid transcript; R=6/13 expected byte-equal to one-shot, R=0/1/3 differ only in trailing punctuation | TODO (Target: MUST PASS — pending user sign-off; predecessor shipped 4-setting equivalent at PASS) |
| nemotron-3.5-asr-streaming-0.6b | Word timestamps | only if exposed | `transcribe-cli --timestamps word -m <gguf> samples/jfk.wav` | per-word `t0_ms`/`t1_ms` in JSON output via the family-wide RNN-T emit-frame derivation | TODO (Target: MUST PASS — `EncDecRNNTBPEModelWithPrompt` injects the 128-d prompt into the RNN-T joint without touching the encoder per-frame time axis or the joint emit-frame structure, so the family-wide emit-frame path applies unchanged; `aux_ctc` head also present as a CTC-argmax fallback) |
